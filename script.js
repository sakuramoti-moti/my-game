...
/**
 * ステージ初期化
 * levels.js の STAGE_DATA から情報を読み込みます
 */
function initStage(stgIndex) {
    currentStage = stgIndex;
    // levels.jsに定義されたデータを取得
        ...
        row.forEach((cell, colIndex) => {
            ...
            if (player.x < bx + blockSize && player.x + player.width > bx && player.y < by + blockSize && player.y + player.height > by) {
                if (cell === 1) { // 地面
                    ...
                    setTimeout(() => {
                        // 次のステージがあるかチェック
                        if (currentStage + 1 < STAGE_DATA.length) {
                            initStage(currentStage + 1);
                        } else {
                            msgElement.innerHTML = "ALL CLEAR!"; 
...
function draw() {
    ...
    level.forEach((row, rowIndex) => {
        row.forEach((cell, colIndex) => {
            const bx = colIndex * blockSize, by = startY + rowIndex * blockSize;
            if (cell === 1) {
                // 地面の色（ステージごとに変える場合はここを調整）
                ctx.fillStyle = (currentStage === 0) ? "#7AC143" : (currentStage === 1) ? "#2d5a27" : "#333";
